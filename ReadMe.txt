...
- **Color Change:** In the `PlayerController.cs` script, I keep a reference to the player's `SpriteRenderer`. Inside the `Jump()` method, I simply set `spriteRenderer.color` to a designated `jumpColor` (cyan). Inside my `CheckGrounded()` logic, which uses `Rigidbody2D.GetContacts()` to accurately detect floor collisions, I reset the color back to `originalColor` as soon as the player lands.
